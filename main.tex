...
\note{Chèn sơ đồ kiến trúc YOLOv8 ở đây (backbone CSPDarknet $\to$ PAN neck $\to$ Detect head). Có thể vẽ bằng draw.io hoặc dùng hình từ tài liệu Ultralytics.}
...
\note{Chèn confusion matrix hoặc ảnh failure cases ở đây. Có thể lấy confusion matrix từ thư mục runs/exp*/confusion\_matrix.png.}
...
\note{Chèn biểu đồ visualization từ \texttt{scripts/visualize\_official\_results.py} ở đây (ood\_map\_by\_zone.png, id\_ood\_tradeoff.png). Chạy lệnh: \texttt{python scripts/visualize\_official\_results.py}}
