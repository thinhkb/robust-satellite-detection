...
try:
    from train_dg_aug import DGAugTrainer, build_dg_aug_transforms
except ImportError:
    from scripts.train_dg_aug import DGAugTrainer, build_dg_aug_transforms
...
class ACSDetectionTrainer(DetectionTrainer):
    ...
    def build_dataset(self, img_path, mode="train", batch=None):
        if mode == "train" and ACS_CONTEXT.get("use_custom_dg_aug", False):
            ...
            if not supports_custom:
                raise RuntimeError(
                    "Installed Ultralytics cannot inject custom DG-Aug transforms. "
                    "Install the version pinned in requirements.txt."
                )
            self.args.augmentations = build_dg_aug_transforms()
            LOGGER.info("Custom DG-Aug enabled for ACS-YOLO final stage.")
...
def train(args: argparse.Namespace) -> Path:
    ...
    if args.warmup_epochs > 0:
        ...
        if warmup_weights.exists() and not args.force_retrain:
            ...
            warmup_trainer = DGAugTrainer if args.warmup_aug == "dg_aug" else None
            warmup_kwargs = yolo_train_kwargs(
                args,
                cfg_path=cfg_path,
            ...
            if warmup_trainer is None:
                warmup_model.train(**warmup_kwargs)
            else:
                warmup_model.train(
    ...
    ACS_CONTEXT.update(
        ...
        use_custom_dg_aug=args.final_aug == "dg_aug",
    )
